...
//	dalitz_km_pi = new TH2I("dalitz_km_pi",  "kpkm_kpkmpi0", 200, 0, 4, 200, 0, 4);
	...
	hAngle_GJ_Kstar_Kminus_theta_phi = new TH2D("cos_GJ_angle_kstar_kminus_theta_phi", "cos_GJ_angle_kstar_kminus_theta_phi", 100, -3.2, 3.2, 100, -1, 1);

	hAngle_TY_Phi_pi0 = new TH2D("TY_angle_phi_pi0", "TY_angle_phi_pi0", 150, 1, 4, 361, -180, 180);

	hAngle_cosTheta_Phi_sim_check = new TH2D("cosTheta_Phi_sim_check", "cosTheta_Phi_sim_check", 100, -3.2, 3.2, 100, -1, 1);

	hAngle_cosTheta_Phi_sim_replicate = new TH2D("cosTheta_Phi_sim_replicate", "cosTheta_Phi_sim_replicate", 100, -3.2, 3.2, 100, -1, 1);
...
//		cout << "KPlus: " <<  dKPlusWrapper->Get_Charge() << dKPlusWrapper->Get_TrackID() ;
...
//
		...
		if(lockppi0P4.M() > 0.75 && lockppi0P4.M() < 0.90) {
			...
			Double_t phi_p= locDecayingKstarpP4_GJ.Phi();

			hAngle_GJ_Kstar_Kminus->Fill(loc2kpi0P4.M(), cos(theta_p));

			hAngle_GJ_Kstar_Kminus_theta_phi->Fill(phi_p, cos(theta_p));
		...
		resonance = locKPlusP4 + locKMinusP4 + locDecayingPi0P4_Measured;

		TLorentzRotation resonanceBoost(-resonance.BoostVector());

		TLorentzVector resonance_X = resonance;

		resonance_X.Boost(-gj_X_Kstarp_Km_boost_vec);


		TLorentzVector locProtonP4_res = locProtonP4;
		locProtonP4_res.Boost(-gj_X_Kstarp_Km_boost_vec);
...
//		locDecayingKstarpP4->
        ...
        TVector3 z = -1. * locProtonP4_res.Vect().Unit();
        TVector3 x = y.Cross(z).Unit();

		TLorentzVector  vec_Kstar_plus_res = resonanceBoost * locDecayingKstarpP4;

        TVector3 Kstar_plus_angles( (vec_Kstar_plus_res.Vect()).Dot(x),
                       (vec_Kstar_plus_res.Vect()).Dot(y),
                       (vec_Kstar_plus_res.Vect()).Dot(z));

		hAngle_cosTheta_Phi_sim_check->Fill(Kstar_plus_angles.Phi(), Kstar_plus_angles.CosTheta());	

//		cout << "Y: " << y.X() << "  " << y.Y() << "  " << y.Z() << endl;
//		cout << "Z: " << z.X() << "  " << z.Y() << "  " << z.Z() << endl;
//		cout << "X: " << x.X() << "  " << x.Y() << "  " << x.Z() << endl;
//
//		cout << "Angle: " << Kstar_plus_angles.X()*180/pi_const << "  " << Kstar_plus_angles.Y()*180/pi_const << "  " <<Kstar_plus_angles.Z()*180/pi_const << endl;
//
//		cout << vec_Kstar_plus_res.Vect().Angle(x) << endl;
//		cout << vec_Kstar_plus_res.Vect().Angle(y) << endl;
//		cout << vec_Kstar_plus_res.Vect().Angle(z) << endl;
//
//
//		cout << "cc:  "<< vec_Kstar_plus_res.Vect().Angle(Kstar_plus_angles)*180/pi_const << endl;

		/*--------------------------------------------------*/
		/// replicating the simulation output

		// hAngle_cosTheta_Phi_sim_replicate	
	    // Double_t theta_p= locDecayingKstarpP4_GJ.Theta();
		// Double_t phi_p= locDecayingKstarpP4_GJ.Phi();


		//cout << gj_X_Kstarp_Km_boost_vec.X() << "  " << gj_X_Kstarp_Km_boost_vec.Y() << "  " << gj_X_Kstarp_Km_boost_vec.Z() << endl;
		// cout << resonanceBoost.X() << "  " <<  resonanceBoost.Y() << "  " << resonanceBoost.Z() << endl;
		//cout << resonance.BoostVector().X() << "  " <<  resonance.BoostVector().Y() << "  " << resonance.BoostVector().Z() << endl;


//		cout << "Sim cos(Theta): " << Kstar_plus_angles.CosTheta()        << "     Sim Phi: " <<  Kstar_plus_angles.Phi() << endl;
//		cout << "cos(Theta): "     << cos(locDecayingKstarpP4_GJ.Theta()) << "     Phi: " << locDecayingKstarpP4_GJ.Phi() << endl;
//
//
//		locDecayingKstarpP4_GJ = locDecayingKstarpP4 * locDecayingKstarpP4;



//		cout << locDecaying_X_Kstarp_Km_P4.BoostVector().X() << "  " << locDecaying_X_Kstarp_Km_P4.BoostVector().Y() << "  " << locDecaying_X_Kstarp_Km_P4.BoostVector().Z() << endl;
//
//		cout << resonance.BoostVector().X() << "  " << resonance.BoostVector().Y() << "  " << resonance.BoostVector().Z() << endl;
//
//
//		cout << "Res X : " << resonance_X.Theta() * 180 / pi_const << "    Rex X: " << resonance_X.Phi()  * 180 / pi_const<< endl; 
//
//		cout << "Before Theta: " << locDecayingKstarpP4.Theta() * 180 / pi_const << "    Before Phi: " << locDecayingKstarpP4.Phi()  * 180 / pi_const<< endl; 
//
//
//		cout << "Res Theta: " << gj_X_Kstarp_Km_boost_vec.Theta() * 180 / pi_const     << "    Res Phi: " <<  gj_X_Kstarp_Km_boost_vec.Phi()  * 180 / pi_const << endl;
//
//		cout << "K*Plus Res Theta: " << vec_Kstar_plus_res.Theta() * 180 / pi_const     << "     K*Plus Res Phi: " << vec_Kstar_plus_res.Phi()  * 180 / pi_const << endl;
//
//		cout << "K*Plus Theta: "     << locDecayingKstarpP4_GJ.Theta() * 180 / pi_const << "    K*Plus Phi: " << locDecayingKstarpP4_GJ.Phi() * 180 / pi_const<< endl;
//		
//		cout << "Sim Theta: " << Kstar_plus_angles.Theta() * 180 / pi_const     << "     Sim Phi: " << Kstar_plus_angles.Phi() * 180 / pi_const<< endl;
//
//		exit(0);

		hAngle_cosTheta_Phi_sim_replicate->Fill(locDecayingKstarpP4_GJ.Phi(), cos(locDecayingKstarpP4_GJ.Theta()));
//		hAngle_cosTheta_Phi_sim_replicate->Fill(locDecayingKstarpP4_GJ.Phi(), cos(locDecayingKstarpP4_GJ.Theta()));
