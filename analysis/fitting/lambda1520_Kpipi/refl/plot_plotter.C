...
void plot_plotter(TString dir = "./") {

	gStyle->SetOptStat(0);

	const int maxPlots = 9;
	TString plotNames[maxPlots] = {"MVecPs","CosTheta","Phi","CosTheta_H","Phi_H","Prod_Ang","MRecoil","MProtonPs","MRecoilPs"};
	const int maxAmps = 3;
    ...
    TFile *f = TFile::Open(dir+"/vecps_plot.root");
	...
	for(int i=0; i<maxPlots; i++) {
		TH1F *hdat = (TH1F*)f->Get(plotNames[i]+"dat");
		...
		for(int j=0; j<maxAmps; j++) {
            TH1F *hacc = (TH1F*)f->Get(plotNames[i]+"acc"+ampNames[j]);
