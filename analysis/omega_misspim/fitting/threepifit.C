...
void threepifit(bool mc = true, bool save = true, const char *whichcut = "pv") {
  TFile* fhist;
  TFile* outROOT;
  if(mc == false) {
    fhist = TFile::Open("/sciclone/gluex10/amschertz/omega_misspim/test7_full/hist_total.root");
    if(save == true)
      outROOT = new TFile(Form("method2_data_%scut.root", whichcut), "recreate");
  }
  else {
    fhist = TFile::Open("/sciclone/gluex10/amschertz/omega_misspim/geant4/test2/hist_total.root");
    if(save == true)
