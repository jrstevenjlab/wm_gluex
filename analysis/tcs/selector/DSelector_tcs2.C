...
void DSelector_tcs2::Init(TTree *locTree)
{
 ...
 //identical to dHist_phi except fill after setting cos(theta) range
	...
	for(int loc_i = 0; loc_i < 2; loc_i++) {
		...
		dHist_SigTrans_SigTheta_JpsiACcuts3[loc_i] = new TH2I(Form("%sSigTrans_SigTheta_JpsiACcuts3",locParticleLabel[loc_i].Data()), Form(";%s #sigma_{theta}; %s #sigma_{trans}", locParticleLabel[loc_i].Data(), locParticleLabel[loc_i].Data()), 100, locMinSigTheta, locMaxSigTheta, 100, locMinSigTrans, locMaxSigTrans);
		...
		dHist_SigTrans_SigTheta_JpsiACcuts5[loc_i] = new TH2I(Form("%sSigTrans_SigTheta_JpsiACcuts5",locParticleLabel[loc_i].Data()), Form(";%s #sigma_{theta}; %s #sigma_{trans}", locParticleLabel[loc_i].Data(), locParticleLabel[loc_i].Data()), 100, locMinSigTheta, locMaxSigTheta, 100, locMinSigTrans, locMaxSigTrans);
...
Bool_t DSelector_tcs2::Process(Long64_t locEntry)
{
 ...
 // dividing through by time for 3-vector 
		...
		double loctheta = loc_eeP3_Angles.Theta();
