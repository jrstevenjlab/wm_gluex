...
void threepifit(bool mc = true, bool save = true, const char *whichcut = "pv", TString datadir = "/sciclone/gluex10/amschertz/omega_misspim/test7_full/", TString mcdir = "/sciclone/gluex10/amschertz/omega_misspim/geant4/test2/") {
  TFile* fhist;
  TFile* outROOT;
  if(mc == false) {
    fhist = TFile::Open(datadir + "hist_total.root");
    if(save == true)
      outROOT = new TFile(Form("method2_data_%scut.root", whichcut), "recreate");
  }
  else {
    fhist = TFile::Open(mcdir + "hist_total.root");
    if(save == true)
      outROOT = new TFile(Form("method2_gen_%scut.root", whichcut), "recreate");
  }
