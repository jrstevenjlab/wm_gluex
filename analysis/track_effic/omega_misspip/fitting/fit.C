...
void fit(bool mc = false, bool save = false, const char *whichcut = "pv") { //whichcut = "pv", "theta", or "p"
  TFile* fhist;
  TFile* outROOT;

  if(mc == false) {
    fhist = TFile::Open("/sciclone/gluex10/amschertz/omega_misspip/test20_full/hist_total.root");
    if(save == true)    
      outROOT = new TFile(Form("mmop_data_%scut.root", whichcut), "recreate");
  }
  else {
    fhist = TFile::Open("/sciclone/gluex10/amschertz/omega_misspip/geant4/test5/hist_total.root");
    if(save == true)
      outROOT = new TFile(Form("mmop_g4_%scut.root", whichcut), "recreate");
  }

  for(int im = 0; im < 7; im++){ //loop over cuts on P(chi^2)
    locHist_MMOP_phi2D[im] = (TH2F*)fhist->Get(Form("OmegaMassVsPhi_%scut%d", whichcut, im+1));
    locHist_MMOP_phi2D[im]->Sumw2();
    locHist_MMOP_phi2D[im]->RebinX(10);
    locHist_MMOP_phi2D[im]->RebinY(30);

    locHist_MMOP_theta2D[im] = (TH2F*)fhist->Get(Form("OmegaMassVsTheta_%scut%d", whichcut, im+1));
    locHist_MMOP_theta2D[im]->Sumw2();
    locHist_MMOP_theta2D[im]->RebinX(10);
    locHist_MMOP_theta2D[im]->RebinY(30);

    locHist_MMOP_p2D[im] = (TH2F*)fhist->Get(Form("OmegaMassVsP_%scut%d", whichcut, im+1));
    locHist_MMOP_p2D[im]->Sumw2();
    locHist_MMOP_p2D[im]->RebinX(10);
    locHist_MMOP_p2D[im]->RebinY(30);
  }

  locHist_MMOP_phi2D_0track = (TH2F*)fhist->Get("OmegaMassVsPhi_missing");
  locHist_MMOP_phi2D_0track->Sumw2();
  locHist_MMOP_phi2D_0track->RebinX(10);
  locHist_MMOP_phi2D_0track->RebinY(30);

  locHist_MMOP_theta2D_0track = (TH2F*)fhist->Get("OmegaMassVsTheta_missing");
  locHist_MMOP_theta2D_0track->Sumw2();
  locHist_MMOP_theta2D_0track->RebinX(10);
  locHist_MMOP_theta2D_0track->RebinY(30);

  locHist_MMOP_p2D_0track = (TH2F*)fhist->Get("OmegaMassVsP_missing");
  ...
  for(int im = 0; im < 7; im++){//loop over mm^2 cuts
    for(int i = 0; i < 20; i++){//loop over phi bins
      cphi[im]->cd(i+1);
	...
	for (Int_t j=0; j<4; j++){
      ...
      if(im == 0 && i == 10){
	TCanvas* cm = new TCanvas("cm", "cm", 500, 500);
	locHist_MMOP_phi->Draw();
	polBG->Draw("same");
	sgfplot->Draw("same");
	sgf->Draw("same");
	cm->Print("plots/mmopexamplefit.C");
      }
  ...
  //fit theta (reco)
  for(int im = 0; im < 7; im++){ //loop over mm^2 cuts
    for(int i = 0; i < 20; i++){ //loop over theta bins
      ctheta[im]->cd(i+1);
	...
	for (Int_t j=0; j<4; j++){
      ...
      // double intHist = locHist_MMOP_theta->Integral();
  ...
  cout << "p fits:" << endl;
  for(int im = 0; im < 7; im++){ //loop over mm^2 cuts
    for(int i = 0; i < 12; i++){ //loop over p bins
      cp[im]->cd(i+1);
  ...
  for(int i = 0; i < 9; i++){
    ...
    locHist_theta2D_0track_p[i] = (TH2F*)fhist->Get(Form("OmegaMassVsTheta_missing_p%d", i+1));
  ...
  for(int im = 0; im < 7; im++){
    c1[im] = new TCanvas(Form("c1_%d", im+1), "omega yields", 900, 300);
    c1[im]->Divide(3,1);
    c1[im]->cd(1);
    locHist_phi_denom[im]->SetMinimum(0);
    locHist_phi_reco[im]->SetLineColor(kGreen+3);
    locHist_phi_denom[im]->SetLineColor(kMagenta+3);
    locHist_phi_reco[im]->SetMarkerColor(kGreen+3);
    locHist_phi_denom[im]->SetMarkerColor(kMagenta+3);
    locHist_phi_reco[im]->SetMarkerStyle(kOpenCircle);
    locHist_phi_denom[im]->SetMarkerStyle(kFullCircle);
    locHist_phi_denom[im]->Draw();
    locHist_phi_reco[im]->Draw("same");
    c1[im]->cd(2);
    locHist_theta_denom[im]->SetMinimum(0);
    locHist_theta_reco[im]->SetLineColor(kGreen+3);
    locHist_theta_denom[im]->SetLineColor(kMagenta+3);
    locHist_theta_reco[im]->SetMarkerColor(kGreen+3);
    locHist_theta_denom[im]->SetMarkerColor(kMagenta+3);
    locHist_theta_reco[im]->SetMarkerStyle(kOpenCircle);
    locHist_theta_denom[im]->SetMarkerStyle(kFullCircle);
    locHist_theta_denom[im]->Draw();
    locHist_theta_reco[im]->Draw("same");
    c1[im]->cd(3);
    locHist_p_denom[im]->SetMinimum(0);
    locHist_p_reco[im]->SetLineColor(kGreen+3);
    locHist_p_denom[im]->SetLineColor(kMagenta+3);
    locHist_p_reco[im]->SetMarkerColor(kGreen+3);
    locHist_p_denom[im]->SetMarkerColor(kMagenta+3);
    locHist_p_reco[im]->SetMarkerStyle(kOpenCircle);
    locHist_p_denom[im]->SetMarkerStyle(kFullCircle);
    locHist_p_denom[im]->Draw();
    locHist_p_reco[im]->Draw("same");
  }
  
  
  TCanvas* c2[7];
  for(int im = 0; im < 7; im++){
    c2[im] = new TCanvas(Form("c2_%d", im+1), Form("Omega yield vs theta, p (%dth cut)", im), 900, 900);
    c2[im]->Divide(3, 3);
    for(int i = 0; i < 9; i++){
      c2[im]->cd(i+1);
      locHist_yield_theta_denom_p[i][im]->SetMinimum(0);
      locHist_yield_theta_denom_p[i][im]->SetMarkerStyle(kFullCircle);
      locHist_yield_theta_denom_p[i][im]->SetMarkerColor(kMagenta+3);
      locHist_yield_theta_denom_p[i][im]->SetLineColor(kMagenta+3);
      locHist_yield_theta_p[i][im]->SetMarkerStyle(kOpenCircle);
      locHist_yield_theta_p[i][im]->SetMarkerColor(kGreen+3);
      locHist_yield_theta_p[i][im]->SetLineColor(kGreen+3);
  ...
  // TCanvas* c3 = new TCanvas("c3", "Error vs theta, p (1 track)", 900, 900);
  // c3->Divide(3,3);
  // for(int i = 0; i < 9; i++){
  //   c3->cd(i+1);
  //   locHist_error_1track[i]->Draw();
  // }

  // TCanvas* c4 = new TCanvas("c4", "Error vs theta, p (0 tracks)", 900, 900);
  // c4->Divide(3,3);
  // for(int i = 0; i < 9; i++){
  //   c4->cd(i+1);
  //   locHist_error_0track[i]->Draw();
  // }
