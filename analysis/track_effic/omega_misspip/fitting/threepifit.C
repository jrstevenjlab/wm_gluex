...
double fit1gaus(double *x, double *par) { //Single Gaussian plus cubic polynomial
  return par[0]*TMath::Exp(-0.5*TMath::Power((x[0]-par[1])/par[2], 2.0)) + par[3] + par[4]*x[0] + par[5]*x[0]*x[0] + par[6]*x[0]*x[0]*x[0];
}

double fit4gaus(double *x, double *par) { //4 Gaussians, 2 with same centroid, plus cubic polynomial
...
double omega2gaus(double *x, double *par) { //2 Gaussians with same centroid
  return par[0]*TMath::Exp(-0.5*TMath::Power((x[0]-par[1])/par[2], 2.0)) + par[3]*TMath::Exp(-0.5*TMath::Power((x[0]-par[1])/par[4], 2.0));
}

double sgf1(double *x, double *par) { //Single asymmetric Gaussian plus cubic polynomial
...
double sgf1plot(double *x, double *par) { //Single asymmetric Gaussian
  return par[0]*exp(-0.5*TMath::Power((x[0]-par[1])/(par[2]+(x[0]<par[1])*par[3]*(x[0]-par[1])),2));
}

void threepifit(bool mc = false, bool save = false, const char *whichcut = "pv") { //whichcut = "pv", "theta", or "p"
  TFile* fhist;
  TFile* outROOT;
  if(mc == false) {
    fhist = TFile::Open("/sciclone/gluex10/amschertz/omega_misspip/test20_full/hist_total.root");
    if(save == true)
      outROOT = new TFile(Form("method2_data_%scut.root", whichcut), "recreate");
  }
  else {
    fhist = TFile::Open("/sciclone/gluex10/amschertz/omega_misspip/geant4/test5/hist_total.root");
    if(save == true)
      outROOT = new TFile(Form("method2_g4_%scut.root", whichcut), "recreate");
  }
  ...
  TH2F* locHist_diff_phi2D = (TH2F*)fhist->Get("OmegaMassVsPhi_missing");
  locHist_diff_phi2D->Sumw2();
  locHist_diff_phi2D->RebinX(10);
  locHist_diff_phi2D->RebinY(30);
  TH2F* locHist_diff_theta2D = (TH2F*)fhist->Get("OmegaMassVsTheta_missing");
  locHist_diff_theta2D->Sumw2();
  locHist_diff_theta2D->RebinX(10);
  locHist_diff_theta2D->RebinY(30);
  TH2F* locHist_diff_p2D = (TH2F*)fhist->Get("OmegaMassVsP_missing");
  ...
  for(int im = 0; im < 7; im++){
	...
	for (Int_t j=0; j<5; j++){
      ...
      if(im == 0 && i == 10){
      	TCanvas* c3pi = new TCanvas("c3pi", "c3pi", 500, 500);
      	locHist_3Pi->Draw();
      	polBG->Draw("same");
      	gauseta->Draw("same");
      	gausphi->Draw("same");
      	...
      	gaus2->Draw("same");
      	gaus4pol3->Draw("same");
      	c3pi->Print("plots/threepiexamplefit.C");
      }
  ...
  for(int im = 0; im < 7; im++){
	...
	gaus4pol3->SetParameter(5, reta->Parameter(0));//FIX THIS LATER
	gaus4pol3->FixParameter(6, reta->Parameter(1));
	gaus4pol3->SetParameter(7, reta->Parameter(2));
      }
      gaus4pol3->SetParameter(8, rphi->Parameter(0));
  ...
  for(int im = 0; im < 7; im++){
    for(int i = 0; i < 12; i++) {
      ...
      gaus4pol3->SetParameter(3, romega->Parameter(0));
      gaus4pol3->SetParameter(4, 2.0 * romega->Parameter(2));
      gaus4pol3->SetParameter(5, 100.);//reta->Parameter(0));
      gaus4pol3->FixParameter(6, 0.55);//reta->Parameter(1));
      gaus4pol3->SetParameter(7, 0.1);//reta->Parameter(2));
      gaus4pol3->SetParameter(8, rphi->Parameter(0));
      gaus4pol3->FixParameter(9, rphi->Parameter(1));
      gaus4pol3->SetParameter(10, rphi->Parameter(2));
      gaus4pol3->SetParameter(11, 1.);
